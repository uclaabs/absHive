...
public class ConditionComputation extends UDAFComputation{
  ...
  public void partialTerminate(int level, int start, int end) {
    ...
    if(this.flag)
    {
      ...
      newCondRanges[level * 2 + 1] = (end == this.rangeMatrix.get(level).size()) ? Double.POSITIVE_INFINITY : this.rangeMatrix.get(level).get(end).getValue(this.flag);
    }
    else
    {
      newCondRanges[level * 2] = (end == this.rangeMatrix.get(level).size()) ? Double.NEGATIVE_INFINITY : this.rangeMatrix.get(level).get(end).getValue(this.flag);
  ...
  public void terminate() {
    System.out.println("Condition Terminate: ");
    
    // TODO Auto-generated method stub
    for(int i = 0; i < this.dimension; i ++)
    {
      System.out.println(newCondRanges[2 * i] + "\t" +  newCondRanges[2 * i + 1]);
      this.condGroup.getRangeMatrix().get(i).add(new ConditionRange(newCondRanges[2 * i], newCondRanges[2 * i + 1]));; 
    }
  }
